...
try {
  ...
  assert.equal(runtimeReplayReferenceIndex.references.mountedFiles.path, "files/mounted-files.json")
  assert.equal(runtimeReplayReferenceIndex.replay.status, "partial")
  assert.ok(runtimeReplayReferenceIndex.replay.instructions.some((instruction: string) => instruction.includes("references.mountedFiles")))
